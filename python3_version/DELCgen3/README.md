...
_Changes on the DELCgen.py code by Davide Cerasole._

_As the code in the [https://github.com/samconnolly/DELightcurveSimulation] is in Python 2, I just updated it to Python 3 and added a couple of useful functions that can be used to fit the power spectral density of the lightcurve: (Gaussian, BendingPL_plus_Gaussian, PL_no_offset, PL_with_offset)_

**All the credits for the lightcurve simulation code are to be given to Sam Connolly and his collaborators.** _This code is identical to DELCgen.py, except for the fact that it is updated to Python 3._

_In ../notebooks folder, I added some notebooks to show how the code can be used, mentioning the important caveats that need to be considered for a proper usage._

_The rest of this README is identical to the README of the original code, except for a couple of necessary changes in the Installation section._
...
_The dependencies of the code are numpy, scipy, matplotlib and astropy. If the DELCgen3 folder has path <path_to_DELCgen3>/DELCgen3/, it is possible to load the modules and classes within DELCgen3 with:_
```python
import sys
sys.path.append("<path_to_DELCgen3>/DELCgen3/")
