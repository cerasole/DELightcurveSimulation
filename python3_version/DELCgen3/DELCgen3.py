...
Changes on the DELCgen.py code by Davide Cerasole

As the code in the https://github.com/samconnolly/DELightcurveSimulation is in Python 2,
I just updated it to Python 3 and added a couple of useful functions that can be used
to fit the power spectral density of the lightcurve:

 - myGauss
 - BendingPL_plus_Gaussian
 - PL_no_offset
 - PL_with_offset

All the credits for the lightcurve simulation code are to be given to Sam Connolly.
This code is purely identical to DELCgen.py, except for the fact that it is updated to Python 3.

In ../notebooks, I added some notebooks to show how the code can be used,
mentioning the important caveats that need to be considered for a proper usage.
...
class Mixture_Dist(object):
    ...
    def __init__(self,functions,n_args,frozen=None, force_scipy=False):
        ...
        if functions[0].__module__ == 'scipy.stats._continuous_distns' or force_scipy == True:
            self.scipy = True
        else:
            self.scipy = False
...
def myGauss(x, A, mu, sigma):
    return A * np.exp( -1. * np.power(x - mu, 2) / (2*sigma**2) )

def BendingPL_plus_Gaussian(v, A, v_bend, a_low, a_high, c, Ag, mu, sigma):
    '''
    Bending power law function - returns power at each value of v,
    where v is an array (e.g. of frequencies)
    ...
    denom = 1 + (v/v_bend)**(a_high-a_low)
    out = A * (numer/denom) + c
    gauss = myGauss (v, Ag, mu, sigma)
    return out + gauss


def PL_no_offset (v, A, index):
    '''
    Power law function - returns power at each value of v,
    where v is an array (e.g. of frequencies)

    inputs:
        v (array)       - input values
        A (float)       - normalisation
        index (float)   - low-frequency index
        c (float)       - intercept/offset
    output:
        out (array)     - output powers
    '''
...
def PL_with_offset (v, A, index, c):
    '''
    Power law function - returns power at each value of v,
    where v is an array (e.g. of frequencies)
...
def TimmerKoenig(RedNoiseL, aliasTbin, randomSeed, tbin, LClength,\
    ...
    rnd.seed(randomSeed)
    real = (np.sqrt(powerlaw*0.5))*rnd.normal(0,1, int(round((RedNoiseL*LClength)/2)))
    imag = (np.sqrt(powerlaw*0.5))*rnd.normal(0,1, int(round((RedNoiseL*LClength)/2)))
    ...
    shortPeriodogram = np.take(periodogram,range(1,int(round(LClength/2+1, 0))))
...
def EmmanLC(time,RedNoiseL,aliasTbin,RandomSeed,tbin,
    ...
    if LClength != None:
        length = LClength
        time = np.arange(0,tbin*LClength,tbin, dtype = "int")
    else:
        length = len(time)
    ...
    while i < maxIterations and np.array_equal(surrogate,oldSurrogate) == False:
        ...
        PSDlast = [periodogram[0],np.take(PSDlast,range(1,int(round(length/2 +1, 0))))]
        ...
        PSDLCAdj = [periodogram[0],np.take(PSDLCAdj, range(1,int(round(length/2 +1, 0))))]
...
class Lightcurve(object):
    ...
    inputs:
        ...
        tbin (int)              - width of time bin of lightcurve
    ...
    def __init__(self, time, flux, tbin, errors = None):
         ...
         self.flux  = flux
         self.errors = errors
         self.length = len(time)                            # numero di punti della LC

         #self.freq = np.arange(1, self.length/2.0 + 1) / (self.length * tbin)
         # The array of frequencies goes from 2*tbin to self.length*tbin, assuming evenly-spaced observations.
         # The Lightcurve object assumes that the lightcurve is evenly binned on a time grid with bin width given by tbin.
         # This is a limitation of the DELCgen original code.

         self.freq = np.arange( 1./(self.length * tbin), \
                                1./(2*tbin) + 1./(self.length * tbin), \
                                1./(self.length * tbin) )
         self.psd = None
    ...
    def STD_Estimate(self, PSDdist = None, PSDdistArgs = None):
        ...
        self.std_est = SD_estimate(self.mean,    \
                                   self.freq[0], \
                                   self.freq[-1],\
                                   PSDdist,      \
                                   PSDdistArgs)[0]
    ...
    def Periodogram(self):
        ...
        shortPeriodogram = np.take(periodogram,np.arange(1, self.length/2 + 1, dtype = "int"))
    ...
    def Fit_PSD(self, initial_params= [1, 0.001, 1.5, 2.5, 0],
        ...
        inputs:
            ...
            bounds (tuple of tuples)
                Sets the fit range for each parameter e.g.  ((1, 100), (1, 100), (1,100)) for a 3-parameter function
        '''
    ...
    def Fit_PDF(self, initial_params=[6,6, 0.3,7.4,0.8,0.2],
        ...
        # Min_PDF is a function defined in this script. It takes as parameters the initial_params and *args, hence the hist and model, in this order.
        m = op.minimize(Min_PDF, initial_params,
                            args=(hist,model),method=fit_method)

        # To be implemented: fit using iminuit + possibility to have more control over the fit procedure.
        # This implementation seems not optimal to me.
            ...
            if verbose:
                ...
                if mix:
                    if model.default == True:
                        ...
                        print ("weight: {}".format(m['x'][4] / (m['x'][4] + m['x'][5])))
                        print ("\nLognormal Function:")
                        print ("exp(ln(mu)): {}".format(m['x'][2]))
                        print ("ln(sigma): {}".format(m['x'][3]))
                        print ("weight: {}".format(m['x'][5] / (m['x'][4] + m['x'][5])))
                    else:
                        for p in range(len(m['x'])):
                            print ("Parameter {}: {}".format(p+1,m['x'][p]))
                else:
    ...
    def Plot_Periodogram(self, show_plot = False):
        '''
        ...
        ax.scatter(self.periodogram[0],self.periodogram[1])
        ...
        ax.set_xlim([0.8*min(self.periodogram[0]),1.2*max(self.periodogram[0])])
        ax.set_ylim([0.8*min(self.periodogram[1]),1.2*max(self.periodogram[1])])
        if show_plot:
            plt.show()
        return fig, ax


    def Plot_Lightcurve(self, show_plot = False):
        '''
        Plot the lightcurve.
        '''
        fig, ax = plt.subplots(figsize = (15, 6))
        ax.errorbar(self.time, self.flux, yerr = self.errors, ls = "None", marker = "o", ms = 4)
        ax.set_title("Lightcurve")
        if show_plot:
            plt.show()
        return fig, ax

    def Plot_PDF(self, bins = None, norm = True, force_scipy = False, show_plot = False):
        '''
        ...
        fig, ax = plt.subplots(figsize = (8, 6))
        ax.hist(self.flux,bins=self.bins, density=norm)
        #kappa,theta,lnmu,lnsig,weight
        if self.pdfFit:
            ...
            if mix:
                ax.plot(x,self.pdfModel.Value(x, self.pdfFit['x']),
                       'red', linewidth=3)
            elif scipy:
                ax.plot(x,self.pdfModel.pdf(x, *self.pdfFit['x']),
                       'red', linewidth=3)
            else:
                ax.plot(x,self.pdfModel(x, *self.pdfFit['x']),
                       'red', linewidth=3)
    ...
    for lc in lightcurves:
        ...
        if bins == None:
            if lc.bins == None:
                lc.bins = OptBins(lc.flux)
            bins = lc.bins
        plt.hist(lc.flux,bins=bins,density=norm,color=colors[c] )
